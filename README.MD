...
pub fn test_eo_trainer(ann : &mut Evonet){
    ...
    let population_size : usize = 100; // set search poplation size,
    let dimensions: usize = ann.get_weights_biases_count(); // get search space dimension, 
    let max_iterations : usize = 1000; // set maximum number of iterations (learning step),
    let lb = vec![-5.0; dimensions]; // set lower bound for the search space,
    let ub = vec![5.0; dimensions]; // set upper bound for the search space,
    let a1 : f64 = 2.0; // give value to a1 parameter (Equilibrium Optimizer),
    let a2 : f64 = 2.0; // give value to a2 parameter (Equilibrium Optimizer),
    let gp :f64 = 0.5; // give value to GP parameter (Equilibrium Optimizer),

    // create EO parameters (learning algorithm)
...
pub fn test_pso_trainer(ann : &mut Evonet){
    ...
    let lb = vec![-5.0; dimensions]; // set lower bound for the search space,
    let ub = vec![5.0; dimensions]; // set upper bound for the search space,
    let c1 : f64 = 2.0; // give value of c1 parameter (PSO),
    let c2 : f64 = 2.0; // give value of c2 parameter (PSO),
